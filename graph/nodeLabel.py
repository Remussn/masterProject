		...
		for i in range(len(self.content_sents_num)):
			...
			for tagged in taggeds:   # create dict
				for ele in tagged:
					...
					t_id = tokenToID[token]
					try:
						self.IDToTag[t_id] += ',' + str(ele[1].encode())
					except:
						self.IDToTag[t_id] = str(ele[1].encode())    # make sure it is string
	...
	def nerTag(self, ner_model):
		...
		for case in switch(ner_model):
			if case('3classes'):
				...
				break
			if case('4classes'):
				path_to_models = '/Users/SunYu/nltk_data/stanford-ner-2016-10-31/classifiers/english.conll.4class.distsim.crf.ser.gz'
				break
			if case('7classes'):
